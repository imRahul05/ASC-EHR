...
> **MindScript reuse caveat:** MindScript code is not in this repo. "Reuse" below means *port the component/engine* into `@repo/ui` / shared packages. Confirm MindScript's stack (React? Tailwind? data model?) — if it matches, it is a copy-and-adapt; if not, it is re-implementation of a proven design (still much cheaper, the iteration is already paid for). See open question Q9 in [05](05-delivery-plan.md).
...
| **M04 Nursing** | Vitals row, signed-record pattern | `Questionnaire`, `Observation`, `Procedure`, `Provenance` (signatures) | Time-out multi-attestation, counts, circulating note content, **one Questionnaire renderer** in `@repo/ui` (reused by M03/M04/M08/M11/X2) | M |
...
| **M08 PACU/Discharge** | Vitals row, note-gen pattern | `Questionnaire`, `Observation`, `CarePlan`, `Communication` | Aldrete/PADSS scoring (`@repo/clinical-rules`), discharge gate, instructions agent, recall entry | S–M |
...
|---|---|---|
| Our own Postgres clinical schema + migrations | Medplum FHIR store (Postgres) + FHIR profiles in `@repo/fhir` | Search, versioning (`_history`), validation, referential model already done; interop in P3 is free |
...
| PHI access audit table | Medplum `AuditEvent` (automatic) + `@repo/audit` for business/agent events | Covers HIPAA access audit without instrumenting every read |
...
| Mantine-based clinical UI (`@medplum/react`) | Headless `@medplum/react-hooks` + `@repo/ui` | Repo rule: shadcn/Base UI only in `apps/web`; avoids two design systems |
